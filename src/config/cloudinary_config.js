 ...
 * @returns {Promise<string>} A promise that resolves to the secure URL of the the uploaded image on Cloudinary.
 * @thorws {Error} If there's an error during the upload process.
 */
...
const uploadToCloudinary = async (image, public_id) => {
    try {
        ...
        console.error('Error uploading imaage to Cloudinary: ', error.message);
        throw error;
    }
}
